    ...
    <div class="container">
        ...
        <div class="main-body">
            <div class="row">
                ...
                <div class="col-lg-12">
                    <form method="post" action="{{ route('operations.tasks.update', $tasks->id)}}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="col-sm-4 text-secondary select2 ">
                                        <select class="single-select @error('project_id') is-invalid @enderror  " id="project_id" name="project_id" >
                                            ...
                                            @foreach($projects as $project)
                                                <option value="{{$project->id}}" {{ old('project_id', $tasks->project_id) == $project->id ? 'selected' : '' }}>{{ $project->name  }}</option>
                                            @endforeach
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-4 text-secondary">
                                        <input type="text" name="task" id="task" class="form-control @error('task') is-invalid @enderror"  value="{{old('task') ?: $tasks->task}}" placeholder="">
                                        @error('task')
                                        <div class="invalid-feedback">
                                            {{$message}}
                                        </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-4 text-secondary">
                                        ...
                                        <input type="date" id="start_date" name="start_date" class="form-control   @error('start_date') is-invalid @enderror" value="{{old('start_date') ?: $tasks->start_date}}"   placeholder="Enter Start Date...">
                                        @error('start_date')
                                        <div class="invalid-feedback">
                                            {{$message}}
                                        </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-4 text-secondary">
                                        <input type="date" class="form-control @error('end_date') is-invalid @enderror " id="end_date" name="end_date" value="{{old('end_date') ?: $tasks->end_date}}" placeholder="Enter End Date...">
                                        @error('end_date')
                                        <div class="invalid-feedback">
                                            {{$message}}
                                        </div>
                                    ...
                                    <div class="col-sm-4 text-secondary select2 ">
                                        <select class="single-select @error('user_id') is-invalid @enderror " id="user_id" name="user_id[]" multiple>
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}" {{ (old('user_id') && in_array( $user->id, old('user_id'))) || (empty(old('user_id')) && in_array( $user->id, $tasks->users->pluck('id')->toArray())) ? 'selected' : ''}}>{{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_id')
                                        <div class="invalid-feedback">
