<?php

namespace App\Http\Controllers\POS;

use App\Http\Controllers\Controller;
use App\Models\Account;
use App\Models\InventoryMovement;
use App\Models\Party;
use App\Models\Product;
use App\Models\Purchase;
use App\Models\PurchaseItem;
use App\Models\Warehouse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Str;

class PurchaseController extends Controller
{
    public function index()
    {
        $products = Product::where('is_active', true)->orderBy('name')->get();
        $suppliers = Party::where('type', 'supplier')->orderBy('name')->get();
        $warehouses = Warehouse::orderBy('name')->get();
        $accounts = Account::orderBy('name')->get();
        return view('pos.purchase', compact('products','suppliers','warehouses','accounts'));
    }

    public function store(Request $request)
    {
        // Filter out empty/incomplete rows before validation
        $filteredItems = collect($request->input('items', []))
            ->filter(function ($row) {
                $pid = $row['product_id'] ?? null;
                $qty = isset($row['quantity']) ? (float)$row['quantity'] : 0;
                $cost = isset($row['cost']) ? (float)$row['cost'] : 0;
                return $pid && $qty > 0 && $cost >= 0;
            })
            ->values()
            ->all();
        $request->merge(['items' => $filteredItems]);
        if (count($filteredItems) === 0) {
            return back()->withErrors(['items' => 'Please add at least one valid item (product, quantity, cost).'])->withInput();
        }

        $data = $request->validate([
            'party_id' => 'nullable|exists:parties,id',
            'warehouse_id' => 'required|exists:warehouses,id',
            'date' => 'required|date',
            'items' => 'required|array|min:1',
            'items.*.product_id' => 'required|exists:products,id',
            'items.*.quantity' => 'required|numeric|min:0.001',
            'items.*.cost' => 'required|numeric|min:0',
            'discount' => 'nullable|numeric|min:0',
            'tax' => 'nullable|numeric|min:0',
            'shipping' => 'nullable|numeric|min:0',
            'payment.amount' => 'nullable|numeric|min:0',
            'payment.account_id' => 'nullable|exists:accounts,id',
            'payment.method' => 'nullable|in:cash,bank_transfer,cheque,card,mobile_wallet',
        ]);

        return DB::transaction(function () use ($data) {
            $subtotal = 0;
            foreach ($data['items'] as $item) {
                $subtotal += ($item['cost'] * $item['quantity']);
            }
            $discount = $data['discount'] ?? 0;
            $tax = $data['tax'] ?? 0;
            $shipping = $data['shipping'] ?? 0;
            $total = max(0, $subtotal - $discount + $tax + $shipping);

            $purchase = Purchase::create([
                'invoice_no' => 'P-' . Str::upper(Str::random(8)),
                'party_id' => $data['party_id'] ?? null,
                'warehouse_id' => $data['warehouse_id'],
                'subtotal' => $subtotal,
                'discount' => $discount,
                'tax' => $tax,
                'shipping' => $shipping,
                'total' => $total,
                'paid' => 0,
                'due' => $total,
                'date' => $data['date'],
                'status' => 'received',
            ]);

            foreach ($data['items'] as $item) {
                $lineTotal = ($item['cost'] * $item['quantity']);
                PurchaseItem::create([
                    'purchase_id' => $purchase->id,
                    'product_id' => $item['product_id'],
                    'quantity' => $item['quantity'],
                    'cost' => $item['cost'],
                    'discount' => 0,
                    'tax' => 0,
                    'total' => $lineTotal,
                ]);

                InventoryMovement::create([
                    'product_id' => $item['product_id'],
                    'warehouse_id' => $data['warehouse_id'],
                    'type' => 'in',
                    'quantity' => $item['quantity'],
                    'reference_type' => 'purchase',
                    'reference_id' => $purchase->id,
                    'date' => $data['date'],
                    'note' => 'Purchase ' . $purchase->invoice_no,
                ]);
            }

            if (!empty($data['payment']['amount']) && $data['payment']['amount'] > 0) {
                $purchase->payments()->create([
                    'account_id' => $data['payment']['account_id'],
                    'method' => $data['payment']['method'] ?? 'cash',
                    'amount' => min($data['payment']['amount'], $total),
                    'paid_at' => $data['date'],
                    'reference' => $purchase->invoice_no,
                ]);
                $purchase->paid = $purchase->payments()->sum('amount');
                $purchase->due = max(0, $purchase->total - $purchase->paid);
                $purchase->save();
            }

            return redirect()->back()->with('success', 'Purchase created');
        });
    }

    public function history(Request $request)
    {
        $q = Purchase::with(['party','warehouse'])->orderByDesc('date')->orderByDesc('id');
        if ($request->filled('invoice_no')) {
            $q->where('invoice_no', 'like', '%' . $request->invoice_no . '%');
        }
        if ($request->filled('supplier')) {
            $q->whereHas('party', function($qq) use ($request){
                $qq->where('name','like','%'.$request->supplier.'%');
            });
        }
        if ($request->filled('from')) {
            $q->where('date', '>=', $request->from);
        }
        if ($request->filled('to')) {
            $q->where('date', '<=', $request->to);
        }
        $purchases = $q->paginate(20)->withQueryString();
        return view('pos.purchases_index', compact('purchases'));
    }
}
